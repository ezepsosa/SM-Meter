...
def update_table_custom(analysis, map):
    ...
    if(map['RB'][4] == 1):
        tweets = twitter_tweets(api, map['EN'][1],map['EN'][2],map['EN'][3],map['RB'][2], map['RB'][3])
        for tweet in tweets:
            if tweet.place:
                analysis.insert(parent='', index=0, text='', values=(tweet.text, tweet.user.screen_name, tweet.created_at, tweet.place.country, 'TW'))
            else:
                # If the result does not have any country set, we leave it as unspecified
                analysis.insert(parent='', index=0, text='', values=(tweet.text, tweet.user.screen_name, tweet.created_at, 'Not especified', 'TW'))
    if(map['RB'][5] == 1):
        comments = reddit_comments(reddit,map['EN'][0], map['EN'][2], map['EN'][3],map['RB'][2], map['RB'][3])
        for comment in comments:
            analysis.insert(parent='', index=0, text='', values=(comment.body, comment.author, datetime.utcfromtimestamp(comment.created_utc), 'Not especified', 'RD'))

def twitter_tweets(api,country, query, numItems, eng, spa):
    # QUERY
    if query:
    ...
    # COUNTRY
    if country == '':
        tweets = tweepy.Cursor(api.search_tweets,q=query+" -filter:retweets",lang=language).items(int(numItems))
    else:
        tweets = tweepy.Cursor(api.search_tweets,q=query+" -filter:retweets",lang=language, geocode=COORDINATES[country]).items(int(numItems))
    return tweets

def reddit_comments(reddit, subrdt, query, numItems, eng, spa):
    if subrdt != "":
        subreddit = reddit.subreddit(subrdt)
    else:
        subreddit = reddit.subreddit('all')
    comments = []
    for comment in subreddit.comments(limit=int(numItems)):
