...
from apiSupport import take_secret_credentials, update_table_custom, update_table_real_time
...
class App(Frame):
    ...
    def home(self):
        ...
        Button(self.home, image=advanced_button_image, command = lambda: show_hide_frame(self.home.toggle), border=0, bg='#E4E4E4', activebackground='#E4E4E4').place(x='865', y='68')
    ...
    def realTime(self):
        ...
        def start_real_time_analysis():
            ...
            update_table_real_time(analysis, mapped_vars)
        ...
        Button(self.realTime, image=start_rt_button_image, command = lambda: threading.Thread(target=start_real_time_analysis).start(), border=0, bg='white', activebackground='white').place(x='1060', y='5')
        Button(self.realTime, image=advanced_button_image, command = lambda: show_hide_frame(white_frame.toggle), border=0, bg='white', activebackground='white').place(x='1140', y='5')
        ...
        country_phrase = ttk.Combobox(white_frame,font = ('SegoeUI', 9), state="readonly", values=list(COORDINATES.keys()))
        ...
        white_frame.toggle = [subreddit_label, country_label, query_label, entry_subreddit, country_phrase, entry_query, sfCB, afCB, efCB, spCB, twCB, rdCB, white_frame]  
    ...
    def customAnalysis(self):
        ...
        def start_custom_analysis():
            mapped_vars = {'RB':[sentimentVarSF.get(), aggressiveVarSF.get(), englishVarSF.get(), spanishVarSF.get(), twitterVarSF.get(), redditVarSF.get()],'EN': [entry_subreddit.get(), country_phrase.get(), query_phrase.get(), numItems.get()]}
            num_items = str(mapped_vars['EN'][3])
            if num_items.isdigit():
                update_table_custom(analysis, mapped_vars)
            else:
        ...
        Button(self.customAnalysis, image=start_ca_button_image, command = lambda: threading.Thread(target=start_custom_analysis).start(), border=0, bg='white', activebackground='white').place(x='1060', y='5')
        Button(self.customAnalysis, image=advanced_button_image, command = lambda: show_hide_frame(white_frame.toggle), border=0, bg='white', activebackground='white').place(x='1145', y='5')
